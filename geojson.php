...
$googleSpreadsheetUrl = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSg3imcMqe9N6NhNXH68n4RiqvKWQc6-xvKapzLd69a_XeECDVDjcD1zBf0BsQICF8lXkjg0BrkPzMR/pub?output=csv";
...
if ( !$error && (($handle = fopen($googleSpreadsheetUrl, "r")) !== FALSE) )
{
  ...
  while (($csvRow = fgetcsv($handle, 10000, ",")) !== FALSE)
  {
    ...
    $features[] = array(
      ...
      'properties' => array(
        'title' => $csvRow[2],
        'access_name' => $csvRow[3],
        'water_body' => $csvRow[4],
        'parking_spaces' => $csvRow[5],
        'picnic_shelter' => $csvRow[6],
        'restrooms' => $csvRow[7],
        'handicap_accessible' => $csvRow[8]
      )
    );
